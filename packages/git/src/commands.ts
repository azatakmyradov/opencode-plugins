...
const collectCommitContext = (cwd: string, stageAll: boolean) =>
  Effect.gen(function* () {
    ...
    const log = yield* g.recentLog(cwd).pipe(
      Effect.catchCause(() => Effect.succeed("(no commits yet)")),
    )
    return [
      status || "(no changes)",
      "",
...
const collectPrContext = (cwd: string, base: string) =>
  Effect.gen(function* () {
    ...
    const log = yield* g.recentLog(cwd, 20).pipe(
      Effect.catchCause(() => Effect.succeed("(no commits yet)")),
    )
  ...
  return function run(
    ...
    return Effect.gen(function* () {
      const acquired = yield* Ref.modify(
        busy,
        (running): [boolean, boolean] => (running ? [false, true] : [true, true]),
      )
      if (!acquired) {
...
export function commandSpecs(deps: CommandDeps): CommandSpec[] {
  ...
  return [
    ...
    },
    {
      name: "new-branch",
      description:
        "Generate and create a new git branch programmatically from the current work.",
