...
export class RunController {
  ...
  constructor(parentSignal?: AbortSignal, concurrency = MAX_CONCURRENCY) {
    this.semaphore = new Semaphore(Math.max(1, Math.min(MAX_CONCURRENCY, Math.floor(concurrency))));
    if (parentSignal) {
  ...
  abort(reason = "Workflow was aborted"): void {
  ...
  async settle(options: { abort?: boolean; timeoutMs?: number } = {}): Promise<boolean> {
    this.sealed = true;
