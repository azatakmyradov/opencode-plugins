...
type PreferredCodexEffort = "minimal" | "low" | "medium" | "high" | "xhigh";
...
function resolveCodexBinary(configured?: string): string | undefined {
...
function preview(value: JsonValue | undefined): string | undefined {
  return safeJson(value, PREVIEW_MAX_LENGTH);
}

function previewLine(value: JsonValue | undefined): string | undefined {
  return firstLine(value, PREVIEW_MAX_LENGTH);
}

function protocolError(value: JsonValue | undefined): string {
...
): PreferredCodexEffort | undefined {
  switch (effort) {
    case "off":
    case "minimal":
      return "minimal";
...
function fileChangePreview(item: JsonRecord): string | undefined {
...
function toolOutput(item: JsonRecord, buffered: string): string | undefined {
...
function toolFailed(item: JsonRecord): boolean {
...
function makeCodexSession(
  ...
  return Effect.gen(function* () {
    const binary = resolveCodexBinary(executablePath);
    if (!binary) {
    ...
    if (task.tools) {
      ...
      return yield* new SpawnError({
        message: `codex cannot restrict tools; use claude for ${
    ...
    const threadResult = yield* Effect.tryPromise({
      try: async () => {
        await request("initialize", {
          clientInfo: {
            name: "opencode-external-subagents",
            title: "OpenCode external subagent",
            version: "2.0.0",
          },
...
export function makeCodexBackend(options: CodexBackendOptions = {}): SubagentBackend {
  return {
    name: "codex",
    capabilities: { modelSelection: true, reasoningEffort: true },
    available: Effect.sync(() => resolveCodexBinary(options.executablePath) !== undefined),
    spawn: (task) => makeCodexSession(task, options.executablePath),
  };
}

export const codexBackend = makeCodexBackend();
